...
revenue_with_lag AS (
    select *,
	LAG(revenue_by_month.total_revenue) over (order by order_month asc) as prev_month_sales
	FROM revenue_by_month
),
revenue_lag_perc AS (
	select *, (((total_revenue / prev_month_sales) - 1) * 100) AS growth_perc
...
SELECT
CAST(DATEPART(YEAR, order_month) as varchar) as sales_year,
CAST(DATENAME(MONTH, order_month) as varchar) as sales_month,
CAST(ROUND(growth_perc, 2) as nvarchar) + '%' as revenue_growth
from revenue_growth